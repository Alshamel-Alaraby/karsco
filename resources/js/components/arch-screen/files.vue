...
export default {
    ...
    methods: {
        ...
        favorite(id) {
            ...
            this.favorite_data.user_id = this.$store.state.auth.type == "admin" ? null : this.$store.state.auth.user.id;
            this.favorite_data.admin_id = this.$store.state.auth.type == "admin" ? this.$store.state.auth.partner.id : null;
            let {type, arch_archive_file_id, user_id, admin_id} = this.favorite_data;
            adminApi.put(`/arch-archive-files/toggle-favourite`, {
                type,
