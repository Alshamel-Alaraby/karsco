...
export default {
    page: {
        ...
        meta: [{name: "Installment Condition", content: 'Installment Condition'}],
    },
    mixins:[translation],
    components: {
        Layout,
    ...
    methods: {
        ...
        getData(page = 1) {
            ...
            adminApi.get(`/recievable-payable/rp_installment_condation?page=${page}&per_page=${this.per_page}&search=${this.search}&${filter}`)
                .then((res) => {
                    let l = res.data;
                    this.conditions = l.data;
                    this.conditionsPagination = l.pagination;
        ...
        getDataCurrentPage(page = 1) {
            if (this.current_page <= this.conditionsPagination.last_page && this.current_page != this.conditionsPagination.current_page && this.current_page) {
                ...
                adminApi.get(`/recievable-payable/rp_installment_condation?page=${this.current_page}&per_page=${this.per_page}&search=${this.search}&${filter}`)
                    .then((res) => {
                        let l = res.data;
                        this.conditions = l.data;
                        this.conditionsPagination = l.pagination;
            ...
            if (Array.isArray(id)) {
                Swal.fire({
                    ...
                    if (result.value) {
                        this.isLoader = true;
                        adminApi
                            .post(`/recievable-payable/rp_installment_condation/bulk-delete`, { ids: id })
                            .then((res) => {
                                this.checkAll = [];
                                this.getData();
                                Swal.fire({
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        adminApi
                            .delete(`/recievable-payable/rp_installment_condation/${id}`)
                            .then((res) => {
                                this.checkAll = [];
                                this.getData();
                                Swal.fire({
        ...
        AddSubmit() {
            ...
            if (this.$v.create.$invalid) {
                ...
                this.isLoader = true;
                this.errors = {};
                adminApi.post(`/recievable-payable/rp_installment_condation`, this.create)
                    .then((res) => {
        ...
        editSubmit(id) {
            ...
            if (this.$v.edit.$invalid) {
                ...
                adminApi.put(`/recievable-payable/rp_installment_condation/${id}`, {name, name_e, is_default})
                    .then((res) => {
         ...
         */
        resetModalEdit(id) {
            let module = this.conditions.find(e => id == e.id);
        ...
        log(id) {
            if(this.mouseEnter != id){
                ...
                this.mouseEnter = id;
                adminApi
                    .get(`/recievable-payable/rp_installment_condation/logs/${id}`)
                    .then((res) => {
                        let l = res.data.data;
                        l.forEach((e) => {
        ...
        ExportExcel(type, fn, dl) {
            ...
            setTimeout(() => {
                ...
                if (dl) {
                    ...
                    XLSX.writeFile(wb, fn || (('conditions' + '.' || 'SheetJSTableExport.') + (type || 'xlsx')));
                }
                this.enabled3 = true;
            }, 100);
        },
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row justify-content-between align-items-center mb-2">
                            <h4 class="header-title"> {{ $t('general.conditionsTable') }}</h4>
                                ...
                                <div class="d-fex">
                                    ...
                                    <div class="d-inline-flex align-items-center pagination-custom">
                                        <div class="d-inline-block" style="font-size:13px;">
                                            {{ conditionsPagination.from }}-{{ conditionsPagination.to }} /
                                            {{ conditionsPagination.total }}
                                        </div>
                                        <div class="d-inline-block">
                                            <a
                                                href="javascript:void(0)"
                                                :style="{'pointer-events':conditionsPagination.current_page == 1 ? 'none': ''}"
                                                @click.prevent="getData(conditionsPagination.current_page - 1)"
                                            >
                                                <span>&lt;</span>
                                            </a>
                                            <input
                                            ...
                                            <a
                                                href="javascript:void(0)"
                                                :style="{'pointer-events':conditionsPagination.last_page == conditionsPagination.current_page ? 'none': ''}"
                                                @click.prevent="getData(conditionsPagination.current_page + 1)"
                                            >
                                                <span>&gt;</span>
                                            </a>
                        ...
                        <div class="table-responsive mb-3 custom-table-theme position-relative" ref="exportable_table"
                            ...
                            <table class="table table-borderless table-hover table-centered m-0">
                                ...
                                <tr>
                                    ...
                                    <th v-if="setting.name">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ getCompanyKey('installment_condition_name') }}</span>
                                            <div class="arrow-sort">
                                                <i class="fas fa-arrow-up"
                                                   @click="conditions.sort(sortString('name'))"></i>
                                    ...
                                    <th v-if="setting.name_e">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ getCompanyKey('installment_condition_name_e') }}</span>
                                            <div class="arrow-sort">
                                                <i class="fas fa-arrow-up"
                                                   @click="conditions.sort(sortString('name_e'))"></i>
                                                <i class="fas fa-arrow-down"
                                                   @click="conditions.sort(sortString('-name_e'))"></i>
                                            </div>
                                        </div>
                                    ...
                                    <th v-if="setting.is_default">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ getCompanyKey('installment_condition_default') }}</span>
                                        </div>
                                    </th>
                                ...
                                <tbody v-if="conditions.length > 0">
                                <tr
                                    @click.capture="checkRow(data.id)"
                                    @dblclick.prevent="$bvModal.show(`modal-edit-${data.id}`)"
                                    v-for="(data,index) in conditions"
                                    :key="data.id"
                                ...
                                >
                                    ...
                                    <td v-if="enabled3" class="do-not-print">
                                        ...
                                        <b-modal
                                            ...
                                            :title="getCompanyKey('installment_condition_edit_form')"
                                            title-class="font-18"
                                            body-class="p-4"
                                            :ref="`edit-${data.id}`"
                                            :hide-footer="true"
