...
export default {
    ...
    beforeRouteEnter(to, from, next) {
        ...
        return vm.$router.push({ name: "home" });
            ...
            if (vm.$store.state.auth.work_flow_trees.includes('main contact groups') || vm.$store.state.auth.work_flow_trees.includes('receivable payable')) {
            return true;
            } else {
                return vm.$router.push({ name: "home" });
            }
            ...
            if (Array.isArray(id)) {
                Swal.fire({
                    ...
                    if (result.value) {
                        this.isLoader = true;
                        adminApi
                            .post(`/recievable-payable/rp_main_contact_group/bulk-delete`, {ids: id})
                            .then((res) => {
                                this.checkAll = [];
                                this.getData();
                                Swal.fire({
