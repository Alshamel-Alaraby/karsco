...
export default {
    ...
    beforeRouteEnter(to, from, next) {
        ...
        return vm.$router.push({ name: "home" });
            ...
            if (vm.$store.state.auth.work_flow_trees.includes('customer')  || vm.$store.state.auth.work_flow_trees.includes('real estate') || vm.$store.state.auth.user.type == 'super_admin') {
                return true;
            } else {
                return vm.$router.push({ name: "home" });
            }
